...
void* client_handler(void* arg) {
    ...
    while (1) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            // Manejo de las acciones del jugador
            if (strcmp(buffer, "W") == 0) {
                // Mover la paleta hacia arriba
                if (game->players[i].paddle_position > 0) {
                    game->players[i].paddle_position--;
                }
            } else if (strcmp(buffer, "S") == 0) {
                // Mover la paleta hacia abajo
                if (game->players[i].paddle_position < FIELD_WIDTH - PLAYER_HEIGHT) {
                    game->players[i].paddle_position++;
                }
            }
        }
